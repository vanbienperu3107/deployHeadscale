...
- **Nguồn latency CHÍNH = server tự ping:** mỗi vòng (`POLL_INTERVAL`), `node-dedup` gọi **LocalAPI `POST /localapi/v0/ping`** của sidecar (qua socket chia sẻ) để **tự ping mọi node** (disco) → ghi `node_latency` với `src=collector`. **Không phụ thuộc node có chạy gì hay không.** Cần `TS_AUTHKEY` để sidecar join; chưa có → ping fail nhẹ nhàng, `/stats` vẫn mở (chỉ trống số liệu).
